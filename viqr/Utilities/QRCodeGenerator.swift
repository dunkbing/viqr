...
struct QRCodeGenerator {
    static func generateQRCode(from content: QRCodeContent, with style: QRCodeStyle) -> QRCode.Document {
...
//                    doc.design.style.eyeBorder = QRCode.FillStyle.Solid(borderColor.cgColor)
    ...
    static func saveQRCodeToFile(qrCode: QRCode.Document, fileName: String, fileFormat: QRCodeExportFormat) -> URL? {
        let paths = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)
        guard let documentsDirectory = paths.first else { return nil }

        let fileURL = documentsDirectory.appendingPathComponent(fileName).appendingPathExtension(fileFormat.fileExtension)
