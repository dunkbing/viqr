    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if case .whatsapp(let number, let message) = content.data {
                Group {
                    TextField("Phone Number (with country code, e.g., +1 555 123 4567)", text: Binding(
                        get: { number },
                        set: { newValue in
...
//                    .keyboardType(.phonePad)
            ...
            Text("When scanned, this QR code will open WhatsApp with this contact and pre-filled message.")
                .font(.caption)
                .foregroundColor(.gray)

            Text("Important: Phone number must include country code without spaces or special characters (e.g., +15551234567).")
                .font(.caption)
